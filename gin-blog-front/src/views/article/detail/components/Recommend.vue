    ...
    <div class="grid grid-cols-12 gap-1.5">
      <div v-for="item of recommendList" :key="item.id" class="group col-span-12 lg:col-span-4">
        <RouterLink :to="`/article/${item.id}`">
          <div class="relative h-[150px] f-c-c overflow-hidden rounded bg-neutral-800 lg:h-[200px]">
            <img :src="convertImgUrl(item.img)" :alt="item.title" loading="lazy" class="h-full w-full object-cover opacity-40 transition-600 group-hover:scale-110 group-hover:opacity-80">
