...
<template>
  <div class="card-view card-enter hidden text-center lg:block space-y-5">
    ...
    <div class="flex justify-center">
      <!-- 三项本来就是链接, 但没有任何 hover 反馈, 看不出可点 -->
      <RouterLink to="/archives" class="flex-1 transition-300 hover:text-primary">
        <p>文章</p>
        <p class="text-xl">
          {{ articleCount }}
        </p>
      </RouterLink>
      <RouterLink to="/categories" class="flex-1 transition-300 hover:text-primary">
        <p>分类</p>
        <p class="text-xl">
          {{ categoryCount }}
        </p>
      </RouterLink>
      <RouterLink to="/tags" class="flex-1 transition-300 hover:text-primary">
        <p>标签</p>
        <p class="text-xl">
          {{ tagCount }}
        </p>
    ...
    <div class="flex justify-center text-center">
      <button
        class="h-9 w-7/8 f-c-c transform rounded bg-primary text-white transition-200 ease-in-out hover:scale-105 hover:bg-accent"
    ...
    <div class="text-2xl space-x-4">
      ...
      <a :href="`http://wpa.qq.com/msgrd?v=3&uin=${blogConfig.qq}&site=qq&menu=yes`" target="_blank">
        <span class="inline-block transition-300 hover:text-accent">
