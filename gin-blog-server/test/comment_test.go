...
func TestCommentGetList(t *testing.T) {
	BaseSuccessTest(t, Get("/api/comment/list", router))
}

func TestCommentDelete(t *testing.T) {
	ids := []int{11}
	BaseSuccessTest(t, DeleteJson("/api/comment", ids, router))
}

func TestUpdateCommentReview(t *testing.T) {
	param := map[string]any{"ids": []int{11, 12}, "is_review": 1}
	BaseSuccessTest(t, PutJson("/api/comment/review", param, router))
}
